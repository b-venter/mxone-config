...
### Features  
 ...
 - Server addition typing protection protection:
   - Select DC/Rack from menu list
   - Matching server IP address value with selected network address ensures valid IP addresses are set. (IPv4 and IPv6)
   - Enabling Cassandra checks whether IPv6 is used in the system. If used, requires IPv6 address that is valid for the IPv6 network used by that server. Else verifies IPv4 address matches.
